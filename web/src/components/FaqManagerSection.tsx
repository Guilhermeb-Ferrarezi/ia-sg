  ...
  return (
    <section className={`${active ? "block animate-in fade-in slide-in-from-bottom-4 duration-500" : "hidden"} space-y-8`}>
      <div className="rounded-3xl border border-slate-800 bg-[#0f172a]/80 p-8 shadow-2xl backdrop-blur-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative hidden sm:block">
              ...
              <input
                ...
                className="w-48 rounded-xl border border-slate-800 bg-slate-950 pl-10 pr-4 py-2 text-xs text-slate-100 placeholder:text-slate-500 outline-none focus:border-cyan-500/50 transition-all"
              />
            </div>
        ...
        <form className="flex items-end gap-4 bg-slate-900/40 p-4 rounded-2xl border border-slate-800 shadow-inner" onSubmit={onCreateFaqSubmit}>
          <div className="flex-1 grid gap-4 md:grid-cols-2">
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-800 bg-slate-950 px-4 py-2.5 text-sm text-slate-100 placeholder:text-slate-600 outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20 transition-all"
                required
              />
            </div>
            <div className="space-y-1">
          ...
          <button
            ...
            className="shrink-0 flex items-center gap-2 rounded-lg bg-cyan-500 px-5 py-2.5 text-xs font-black text-slate-950 shadow-lg shadow-cyan-500/20 hover:bg-cyan-400 hover:-translate-y-0.5 active:translate-y-0 transition-all disabled:opacity-60 disabled:pointer-events-none uppercase tracking-widest"
          >
            {faqSubmitting ? (
          ...
          {paginated.length > 0 ? (
            ...
            <div className="col-span-full py-12 flex flex-col items-center justify-center border-2 border-dashed border-slate-800 rounded-3xl bg-slate-900/20">
              ...
              <p className="text-sm font-bold uppercase tracking-widest text-slate-600">
                {searchTerm.trim() ? "Nenhum resultado para essa busca" : "Nenhum FAQ cadastrado ainda"}
              </p>
            </div>
          )}
        </div>
        ...
        {totalPages > 1 && (
          <div className="mt-8 flex items-center justify-between px-2">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">
              {filtered.length} FAQ{filtered.length !== 1 ? "s" : ""} • Página {currentPage}/{totalPages}
            </span>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setPage(Math.max(1, currentPage - 1))}
                disabled={currentPage <= 1}
                className="p-2 rounded-lg border border-slate-800 text-slate-400 hover:bg-slate-800 hover:text-white disabled:opacity-30 transition-all"
              >
